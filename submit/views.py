...
def problem_list(request):
    problems = Problem.objects.all().order_by('-created_at')
    return render(request, 'submit/problem_list.html', {'problems': problems})  # Fixed path


def problem_detail(request, problem_id):
    problem = get_object_or_404(Problem, id=problem_id)
    return render(request, 'submit/problem_detail.html', {'problem': problem})  # Fixed path


def submit_solution(request, problem_id):  # Removed @login_required
    problem = get_object_or_404(Problem, id=problem_id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            submission = form.save(commit=False)
            # Handle user assignment for anonymous users
    ...
    return render(request, 'submit/submit_solution.html', {  # Fixed path
        'form': form, 
        'problem': problem
    })
...
def submission_result(request, submission_id):
    ...
    submission = get_object_or_404(CodeSubmission, id=submission_id)
    test_results = submission.test_results.all().order_by('id')
    return render(request, 'submit/submission_result.html', {  # Fixed path
        'submission': submission,
        'test_results': test_results  
    })
